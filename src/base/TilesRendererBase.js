...
export class TilesRendererBase {
	...
	preprocessNode(tile, parentTile, tileSetDir) {
		...
		if (uri) {
			...
			const isExternalTileSet = Boolean(
		...
		if (isExternalTileSet) {
			downloadQueue
				.add(tile, (tileCb) => {
					...
					const uri = this.preprocessURL
						? this.preprocessURL(tileCb.content.uri)
						: tileCb.content.uri;
			...
			downloadQueue
				.add(tile, (downloadTile) => {
					if (downloadTile.__loadIndex !== loadIndex) {
						return Promise.resolve();
					}

					const uri = this.preprocessURL
						? this.preprocessURL(downloadTile.content.uri)
						: downloadTile.content.uri;
				...
				.then((res) => {
					...
					if (res.ok) {
						...
						throw new Error(
							`Failed to load model with error code ${res.status}`
						);
					}
				})
				.then((buffer) => {
